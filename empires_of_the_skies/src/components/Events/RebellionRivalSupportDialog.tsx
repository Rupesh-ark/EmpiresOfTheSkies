  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 2, flexWrap: "wrap" }}>
        <Chip label={`Rebels: ${rebellion.counterSwords} Swords`} color="error" variant="outlined" sx={{ fontWeight: "bold" }} />
        <Chip label={`${defenderKingdom}: ${defenderSwords} Swords`} color="primary" variant="outlined" sx={{ fontWeight: "bold" }} />
      </Box>

      <Typography variant="subtitle2" sx={{ mb: 1 }}>Support which side?</Typography>
      <ToggleButtonGroup
        value={side}
        exclusive
        onChange={(_, newSide) => { if (newSide !== null) setSide(newSide); }}
        sx={{ mb: 2 }}
      >
        <ToggleButton value="defender" color="primary">Defend {defenderKingdom}</ToggleButton>
        <ToggleButton value="rebel" color="error">Support Rebels</ToggleButton>
      </ToggleButtonGroup>
      ...
      {side && (
        <>
          ...
          <Slider value={regiments} onChange={(_, v) => setRegiments(v as number)} min={0} max={adjustedMaxReg} step={1} marks valueLabelDisplay="auto" disabled={adjustedMaxReg === 0} sx={{ mb: 2 }} />

          <Typography variant="subtitle2" sx={{ mb: 1 }}>
            Levies ({levies} of {adjustedMaxLev} available)
          </Typography>
          <Slider value={levies} onChange={(_, v) => setLevies(v as number)} min={0} max={adjustedMaxLev} step={1} marks valueLabelDisplay="auto" disabled={adjustedMaxLev === 0} sx={{ mb: 2 }} />
        </>
      )}
