 ...
 *     player already has a position in playerOrder
...
describe("alterPlayerOrder — counsellor cost", () => {
  it("consumes 1 counsellor", () => {
    ...
    G.playerOrder = { 1: undefined, 2: undefined, 3: undefined, 4: undefined, 5: undefined, 6: undefined };
...
describe("alterPlayerOrder — player order update", () => {
  it("places the player in the chosen position of playerOrder", () => {
    const G = buildInitialG();
    G.playerOrder = { 1: undefined, 2: undefined, 3: undefined, 4: undefined, 5: undefined, 6: undefined };
    callMove(G, "0", 0);
    expect(G.playerOrder[1]).toBe("0");
  });
  ...
  it("records the player on the action board slot", () => {
    ...
    G.playerOrder = { 1: undefined, 2: undefined, 3: undefined, 4: undefined, 5: undefined, 6: undefined };
    callMove(G, "0", 0);
    expect(G.boardState.alterPlayerOrder[1]).toBe("0");
  });
...
describe("alterPlayerOrder — INVALID_MOVE conditions", () => {
  ...
  it("returns INVALID_MOVE when position is already taken", () => {
    const G = buildInitialG();
    G.playerOrder = { 1: "1", 2: undefined, 3: undefined, 4: undefined, 5: undefined, 6: undefined };
    const result = callMove(G, "0", 0);
    expect(result).toBe(INVALID_MOVE);
  });
  ...
  it("returns INVALID_MOVE when player already has a position", () => {
    const G = buildInitialG();
    G.playerOrder = { 1: undefined, 2: "0", 3: undefined, 4: undefined, 5: undefined, 6: undefined };
    const result = callMove(G, "0", 0);
    expect(result).toBe(INVALID_MOVE);
  });
});
