...
function buildMapWithBuilding(player: PlayerInfo, buildingType: "outpost" | "colony", coords: [number, number]): MapBuildingInfo[][] {
  const emptyCell: MapBuildingInfo = { fort: [], garrisonedRegiments: 0, garrisonedLevies: 0, garrisonedEliteRegiments: 0 };
  const grid: MapBuildingInfo[][] = Array.from({ length: 3 }, () =>
    Array.from({ length: 5 }, () => ({ ...emptyCell }))
  );
  const [x, y] = coords;
  grid[y][x] = { player, buildings: buildingType, fort: [], garrisonedRegiments: 0, garrisonedLevies: 0, garrisonedEliteRegiments: 0 };
  return grid;
}
